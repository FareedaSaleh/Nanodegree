...
from audit import update_name,updat_state_name, mapping, stat_mapping
import cerberus

import schema

OSM_PATH = "C:\Users\Fareeda\Desktop\P3\melbourne_australia.osm"
        ...
        for child in element:
            ...
            if child.tag == 'tag':
                        ...
                        # check to see if the cleaning function returns a value:
                                 if  updat_state_name(child.attrib["v"],stat_mapping):
                            # if it does, add it to the dictionary
                                     way_tag["value"] = updat_state_name(child.attrib["v"],stat_mapping)
                                 else:
                            # if it doesn't, the attribute is uncleanable so move to the next element
                                    continue
                            else:
